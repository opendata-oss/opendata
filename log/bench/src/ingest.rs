...
impl Benchmark for IngestBenchmark {
    ...
    async fn run(&self, bench: Bench) -> anyhow::Result<()> {
        ...
        let mut summary = Summary::new()
            ...
            .add("batch_latency_us_mean", mean(stats.latency_sum_us, stats.latency_count))
        ...
        // Close the log to release the SlateDB fence. Drop every shared clone so
        // the Arc is uniquely owned, since `close` consumes `self`.
        drop(stats);
