...
pub struct SlatedbSnapshot {
    ...
    /// Number of times the manifest writer refused to commit a finished L0
    /// upload because its touched segment had `tree_l0_len >= l0_max_ssts`
    /// (or the per-key variant). With the slot-release optimization in
    /// place, the gate moved from dispatch time to commit time, so this
    /// counter now reflects "compactor is behind" pressure specifically —
    /// upload/manifest head-of-line no longer contributes to the gate.
    pub l0_commit_gated_count: u64,
...
impl SlatedbSnapshot {
    pub fn capture(handle: &Handle) -> Self {
        ...
        Self {
            ...
            l0_commit_gated_count: handle.counter_value("slatedb.db.l0_commit_gated_count"),
    ...
    pub fn delta_since(&self, before: &Self) -> Self {
        Self {
            ...
            l0_commit_gated_count: self
                .l0_commit_gated_count
                .saturating_sub(before.l0_commit_gated_count),
