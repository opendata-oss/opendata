...
async fn poll_for_duration(
    ...
    loop {
        ...
        let (n_records, svc) = one_poll(id, &key, state, &mut cursor).await?;
...
async fn one_poll(
    ...
    *cursor = out.cursor;
    ...
    // Live check (not latched at session start): a continuous session
    // (`session_duration = -1`) spawned during warmup must still record its polls
    // once the measure window opens. Per-poll metrics tolerate partial sessions.
    if state.recording() {
