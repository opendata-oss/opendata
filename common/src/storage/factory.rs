...
impl StorageSemantics {
    ...
    /// Sets the segment (prefix) extractor for SlateDB writers and readers.
    ///
    /// SlateDB persists the extractor's name in the manifest and rejects a
    /// reader, compactor, or writer that opens with a different (or absent)
    /// extractor, so system crates that route records into per-prefix segments
    /// must set the same extractor on every path that opens the database.
