...
mod tests {
    ...
    /// Regression test for RFC 0007 unit 6.3.10 — `SubqueryOp` must
    /// release any reservation the factory grew on a per-outer-step
    /// basis. The planner's `ChildFactory` path (see
    /// `physical::build_subquery`) calls `build_node` per outer step,
    /// which in turn calls `resolve_leaf` — that resolve grows the
    /// parent reservation for label storage but has no paired
    /// `release`. Across many outer steps the reservation accumulates
    /// linearly and trips the per-query cap.
    ///
    /// Shape: a scripted `ChildFactory` that reserves a fixed byte count
    /// **from the parent reservation** at construction time (modelling
    /// `resolve_leaf` exactly) and drops the child at end of factory
    /// scope. After draining the whole outer grid and dropping the
    /// `SubqueryOp`, the parent reservation must be at zero — the fix
    /// must introduce per-factory-invocation tracking that releases on
    /// child drop.
    #[test]
    fn should_release_reservation_across_multiple_factory_invocations() {
        // given: 8 outer steps. A scripted factory grows the parent
        // reservation by `LEAK_BYTES` per call and builds a trivially
        // drained child. `LEAK_BYTES` models the `resolve_leaf`
        // label-storage reservation.
        const OUTER_STEPS: usize = 8;
        const LEAK_BYTES: usize = 10_000;

        let outer_grid = StepGrid {
            start_ms: 100,
            end_ms: 100 + (OUTER_STEPS as i64 - 1) * 60,
            step_ms: 60,
            step_count: OUTER_STEPS,
        };
        let series = mk_labels(1);
        let series_clone = series.clone();
        ...
        let factory: ChildFactory = Box::new(move |_tr, step_ms| {
            // Simulate `resolve_leaf`'s unpaired `try_grow`. The
            // production path holds this reservation on a plan-time
            // `SeriesSchema` which lives inside the factory-built child
            // sub-tree; dropping the child must release it.
            res_clone
                .try_grow(LEAK_BYTES)
                .expect("factory reservation must fit");
            let (ts_arc, inner_grid) = mk_inner_grid(100, 30, step_ms);
            let batches: Vec<StepBatch> = ts_arc
                .iter()
                .enumerate()
        ...
        let mut op = SubqueryOp::new(factory, series, outer_grid, 30, 10, reservation.clone());

        // when: drain the whole outer grid.
        let waker = noop_waker();
        let mut cx = Context::from_waker(&waker);
        loop {
            match op.windows(&mut cx) {
        ...
        // then: every factory-scoped allocation must have been released
        // on child drop — the parent reservation should be back at zero.
        assert_eq!(
            reservation.reserved(),
            0,
            "subquery factory reservation leak: {} bytes still held after draining {} outer \
             steps (expected 0 after child-drop scope releases)",
            reservation.reserved(),
            OUTER_STEPS,
        );
    }
