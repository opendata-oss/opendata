...
async fn main() {
    ...
    let (tsdb, storage) = if read_only {
        ...
        let reader_config = config::ReaderConfig {
            storage: prometheus_config.storage.clone(),
            refresh_interval: std::time::Duration::from_secs(
                prometheus_config.reader.refresh_interval_secs,
            ),
            ..Default::default()
        };
        let reader = TimeSeriesDbReader::open(reader_config)
            .await
            .unwrap_or_else(|e| {
