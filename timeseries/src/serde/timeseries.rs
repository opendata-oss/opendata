...
///
/// This is a convenience wrapper around [`merge_batch_time_series`] for the case
/// of merging exactly one operand into a base value.
pub(crate) fn merge_time_series(base: Bytes, other: Bytes) -> Result<Bytes, EncodingError> {
    merge_batch_time_series(Some(base), &[other])
}
