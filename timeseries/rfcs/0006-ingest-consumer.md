...
`opendata-ingest`. A Go OTel Collector exporter writes OTLP
protobuf through the ingest `Ingestor` to object storage. An
...
ingest crate (RFC ingest-0001) decouples these: data is durable
...
- Define the contract between ingestor (Go, in
...
- Define an embedded ingest consumer in the timeseries service
...
- **Changes to the ingest crate API**
...
A 4-byte payload attached to each `Ingestor::ingest()` call:
...
Each `Ingest()` call produces one entry with one metadata
range, so metadata unambiguously describes its entry even
when the ingestor batches multiple calls into a single data
file.

The expectation is that different signal types use separate
ingest channels (distinct manifest paths), but this is not
...
**Configuration** maps directly to `ingest.IngestorConfig`:
...
1. `ConsumeMetrics` marshals `pmetric.Metrics` to OTLP
   protobuf
2. Calls `ingestor.Ingest([][]byte{proto}, metadata)`
   with metadata `[1, 1, 1, 0]`
3. Awaits `WriteHandle.AwaitDurable(ctx)` before returning

On startup, creates object store + ingestor. On shutdown,
calls `ingestor.Close(ctx)`.
...
### Timeseries Ingest Consumer

An `IngestConsumer` background task runs in
...
Add `ingest_consumer: Option<IngestConsumerConfig>` to
`PrometheusConfig`. When absent (default), no consumer starts.

```rust
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct IngestConsumerConfig {
    /// Must point to same object store as the ingestor(s).
    pub object_store: ObjectStoreConfig,

    /// Must match the ingestor's manifest_path.
...
feature. Adds `ingest` as a dependency of the timeseries
crate.
...
The ingest object store may differ from TSDB storage -- the
ingest queue is an intermediary buffer.

### Observability

Prefix: `opendata_ingest_consumer`.
