...
A stateless buffer consists of multiple components -- called writers -- that accept write entries from an API layer,
accumulate them in memory, and periodically flush batched data files to object storage.
On the read side of the ingestion, a reader reads the batches and makes them available to a writer of a database.
Each writer accepts any data, i.e., no mapping from specific ingest data partitions to specific writer exist.
If a writer fails, any other still running writer can take over the work without running any rebalancing protocol.
A manifest-backed queue coordinates producers (writers) and consumers (readers) in a stateless, crash-safe way.
...
- Specify how the writer makes data available to the reader that inserts the data to the OpenData system.
- Specify how the reader can read the ingested data in the correct ingest order including the fail-over case.


## Non-Goals

- Specify how unacknowledged entries can be re-ingested in case of a re-start.
  Progress handling is the responsibility of the system that uses the writers.
- Specify fail-over for the external system that uses the writers.
...
The queue coordinates writers and readers via a single queue manifest in object storage
(`q-manifest` in the diagram).

The queue producers (used internally by the writers) write the locations of the batches of ingested data
...
The queue consumer (used internally by the reader) reads the queue manifest to know the locations of the
...
The writer provides an API to ingest a vector of opaque byte entries with associated metadata.
The entries are buffered in a batch in ingestion order.
The writer flushes the batches of ingested entries to object storage and appends the locations of the
...
The API of the writer is the following:
```rust
impl Writer {
...
A writer is constructed by calling the method `new()` passing to it the configuration and a clock.
The configurations for the writer are:
...
If this backpressure becomes an issue, new writers can be created to better distribute the load.
...
The reader can use the metadata items to interpret ranges of records in the batch without reading it.
...
Method `close()` flushes unflushed entries and terminates the writer.

### Data Batch Format

A data batch is the unit of data that a writer flushes to object storage.
...
When compression is enabled, the writer first serializes all records into a contiguous buffer
...
On the read side, the reader reads the footer to determine the compression type,
decompresses the record block if needed, and then parses the length-prefixed entries as usual.

Compression is configured per-writer via the `batch_compression` field in `WriterConfig`
...
so the reader can discover and read it.


### Reader

The reader reads the locations of the ingested entries in ingestion order from the queue with the
internal queue consumer (`q-consumer` in the diagram) and returns the batches of entries.

The API of the reader is the following:
```rust
impl Reader {
  pub fn new(config: ReaderConfig, clock: Arc<dyn Clock>) -> Result<Self> { ... }

  pub async fn initialize(&self, last_acked_sequence: Option<u64>) -> Result<()> { ... }

  pub async fn next_batch(&self) -> Result<Option<ReadBatch>> { ... }
...
A reader is constructed by calling `new()` passing to it the configuration and a clock.
After construction, the caller must call `initialize()` before using the reader.
This method increments the epoch in the queue manifest, fencing any previous consumer instance.
Only a consumer whose epoch matches the manifest's current epoch may perform queue operations;
if a new reader starts and calls `initialize()`, any previous reader's subsequent
`next_batch()` or `ack()` calls will fail with a `Fenced` error.

The `last_acked_sequence` parameter controls where the reader starts reading:
- `None` — start from the earliest available entry in the queue.
- `Some(seq)` — resume after sequence `seq`, so the reader positions both its
  acknowledged position and its internal read position at `seq`. The next call
  to `next_batch()` reads sequence `seq + 1`.

The configurations for the reader are:
```rust
...
The reader internally creates a queue consumer and an object store client from the configuration.

A `ReadBatch` contains the deserialized entries, the sequence number, the location of the batch,
and the per-range metadata items attached by the writer(s):
```rust
...
The `Metadata` struct exposes the per-range metadata that writers attach to each batch:
```rust
pub struct Metadata {
    ...
    /// Opaque metadata payload supplied by the caller of `Writer::ingest`.
    pub payload: Bytes,
}
```

By calling `next_batch()` the reader reads the next data batch from object storage via the
queue consumer, deserializes the entries, and returns them as a `ReadBatch`.
The reader maintains an internal read cursor that advances in ingestion order.
...
the reader batches acks and only calls `dequeue()` on the queue consumer every 100 acks.
This acknowledged position is tracked separately from the internal read cursor so the
reader can read ahead while still preserving in-order acknowledgements and resume semantics.
...
By calling `flush()` the caller forces the reader to dequeue all acked entries from the
queue manifest immediately.

If the reader fails, a new reader can be started. It will increment the epoch,
...
When the reader dequeues acknowledged entries from the queue manifest, the corresponding
...
The garbage collector runs as a background task within the reader process. On each cycle it:
...
These fields are also available on `ReaderConfig` so that the reader can configure and
spawn the garbage collector automatically.

### Delivery guarantees

Due to the sequence number attached to each ingested batch and the reader API that allows to
read queue entries depending on the sequence number, exactly-once delivery is possible.

To achieve exactly-once the caller of the reader is required to atomically write the
...
After a failure, the writer can continue at the first batch that was not ingested.
No data is ingested twice.

The delivery guarantees of the entries on the writer-side that were not confirmed to be 
durable before a failure depends on the progress tracking of the caller of the writer.
...
Since we assume one writer per availability zone and a single reader which limits the contention, we decided
against the broker.
...
During the flushing of batches, the writer:
...
If the write fails because a batch with that name already exists, the writer increases the number again, 
