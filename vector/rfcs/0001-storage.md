...
- `NullTerminatedUtf8`: UTF-8 bytes followed by a `0x00` terminator, using FoundationDB tuple
  encoding for escaping:
  - `0x00` in payload → `0x00 0xFF` (escaped null)
  - `0xFF` in payload → `0xFF 0x00` (escaped 0xFF to avoid ambiguity)
  - Terminator is an unescaped `0x00` (i.e., `0x00` not followed by `0xFF`)
...
Posting lists are updated via SlateDB merge operators rather than read-modify-write. This enables
...
│ 1 byte  │   1 byte    │ NullTerminatedUtf8│ MetadataTerm bytes │
└─────────┴─────────────┴───────────────────┴────────────────────┘
```

**Key Fields:**

- `field`: Metadata field name (null-terminated for correct lexicographic ordering)
