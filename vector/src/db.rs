...
impl VectorDb {
    ...
    pub async fn write(&self, vectors: Vec<Vector>) -> Result<()> {
        ...
        let mut write_handle = self.write_coordinator.handle(WRITE_CHANNEL).write(writes).await?;
        write_handle
    ...
    pub async fn flush(&self) -> Result<()> {
        let mut handle = self.write_coordinator.handle(WRITE_CHANNEL).flush(false).await?;
