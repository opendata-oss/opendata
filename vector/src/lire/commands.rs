...
impl FinishMergeCommand {
    pub(crate) fn new(task_id: u64, c: u64) -> Self {
        Self { task_id, c }
    }
}
...
impl VectorDbWriteDelta {
    ...
    pub(crate) fn apply_merge_cmd(
        ...
        self.ctx
            .rebalancer
            .update_counts(&[(cmd.c, c_other_count as i32)]);
        self.ctx.rebalancer.drop_centroid(c_other_id);
    ...
    pub(crate) fn apply_merge_sweep_cmd(
        ...
        self.ctx.rebalancer.update_counts(&[(c_id, added as i32)]);
    ...
    pub(crate) fn apply_finish_merge(
        ...
        self.ctx.rebalancer.finish_merge(cmd.task_id, cmd.c);
        Ok(Arc::new(()))
    }
}
