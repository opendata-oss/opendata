...
pub(crate) enum IndexRebalanceOp {
    ...
    ExecuteSplit { centroid: u64 },
    /// Merge centroid `c` (smaller) into centroid `c_other` (larger, survives).
...
impl IndexRebalancer {
    ...
    pub(crate) fn finish_split(&mut self, task_id: u64, c0: Option<u64>, c1: Option<u64>) {
        self.finish_task(
            task_id,
            vec![c0, c1].into_iter().filter_map(|v| v).collect(),
        );
    }

    pub(crate) fn finish_merge(&mut self, task_id: u64, c: u64) {
        self.finish_task(task_id, vec![c]);
    }

    fn finish_task(&mut self, task_id: u64, centroids: Vec<u64>) {
        self.tasks.deregister(task_id);
        let pending = std::mem::replace(&mut self.pending_rebalance, HashSet::new());
        for centroid in pending.into_iter().chain(centroids.into_iter()) {
            if self.tasks.num_tasks() >= self.opts.merge_threshold_vectors {
    ...
    pub(crate) fn maybe_schedule_split(&mut self, candidate_centroid_id: u64) {
        /*if self.pending_rebalance.contains(&candidate_centroid_id) {
            debug!("centroid is already pending a rebalance. skip");
            return;
        }*/
    ...
    pub(crate) fn maybe_schedule_merge(&mut self, candidate_centroid_id: u64, count: u64) {
        ...
        let v = self
            .centroid_graph
            .get_centroid_vector(candidate_centroid_id)
            .expect(format!("unexpected missing centroid {}", candidate_centroid_id).as_str());
        ...
        self.total_merges += 1;
        self.tasks.register(task_id, &[c, c_other]);
        let mut task = IndexRebalancerTask {
            centroid_graph: self.centroid_graph.clone(),
            coordinator_handle: self.coordinator_handle(),
            opts: self.opts.clone(),
        ...
        debug!("start merge for centroids {}->{}", c, c_other);
        tokio::spawn(async move { task.handle_merge(c, c_other).await });
    }
...
impl IndexRebalancerTask {
    async fn send_finish_split(
        ...
        let mut write_handle = self
            .coordinator_handle
            .try_write(VectorDbWrite::Rebalance(cmd))
            .await
    ...
    async fn send_finish_merge(&self, centroid: u64, c_other: Option<u64>) -> Result<(), String> {
        let cmd = RebalanceCommand::FinishMerge(FinishMergeCommand::new(self.task_id, centroid));
        let mut write_handle = self
            .coordinator_handle
            .try_write(VectorDbWrite::Rebalance(cmd))
            .await
    ...
    async fn handle_split(&mut self, centroid: CentroidEntry) -> Result<(), String> {
        ...
        let mut write_handle = self
            .coordinator_handle
            .try_write(VectorDbWrite::Rebalance(sweep_cmd))
            .await
            .map_err(|e| e.to_string())?;
        write_handle
            .wait(Durability::Applied)
    ...
    /// Execute a merge operation. `c` is the smaller centroid (to be removed) and
    /// `c_other` is the larger centroid (survives). Both were determined by the delta
    /// before dispatching this operation.
    async fn handle_merge(&mut self, c: u64, c_other: u64) -> Result<(), String> {
        // =========================================================================
        // Phase 1: MERGE
        // =========================================================================
        ...
        // Send MergeCommand: merge c's postings into c_other
        let cmd = RebalanceCommand::Merge(MergeCommand::new(
            self.task_id,
            c_other,
            CentroidPostings::new(c, posting_list),
        ));
        ...
        // Swept postings go to c_other (the surviving centroid)
        let sweep_cmd = RebalanceCommand::MergeSweep(MergeSweepCommand::new(
            self.task_id,
            CentroidPostings::new(c_other, swept_postings),
        ));
        ...
        // Re-read c_other's posting list (all vectors from c are now in c_other)
        let reader = ViewReader::new(self.coordinator_handle.view());
        let c_other_postings = reader
            .get_posting_list(c_other, self.opts.dimensions)
            .await
            .map_err(|e| e.to_string())?;

        // Collect all vector IDs that originally belonged to c
        let all_c_ids: HashSet<u64> = c_postings
            .iter()
            .map(|p| p.id())
        ...
        for p in c_other_postings.iter() {
            if !all_c_ids.contains(&p.id()) {
                continue;
            }
            let nearest = self.centroid_graph.search(p.vector(), 1);
            let Some(&nearest_id) = nearest.first() else {
        ...
        async fn new(
            ...
            let ctx = VectorDbDeltaContext {
                opts: VectorDbDeltaOpts {
                    dimensions,
                    chunk_target,
                    max_in_flight_rebalance_tasks: usize::MAX,
