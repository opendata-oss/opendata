...
/// Currently, the reader only supports reading the db at the point-in-time
/// that the reader was opened. To query newer writes the reader must be
/// re-opened.
    ...
    pub async fn open_with_runtime(
        ...
        let storage = create_storage_read(
            ...
            slatedb::config::DbReaderOptions {
                // disable manifest polling because we don't yet populate updated
                // centroids from the reader
                manifest_poll_interval: Duration::MAX,
