...
async fn siftsmall_recall() {
    ...
    // Load the random 100K indices selected by gen_sift100k_groundtruth
    let indices_vecs = read_ivecs(&data_dir.join("sift100k_indices.ivecs"));
    let selected_indices: Vec<usize> = indices_vecs[0].iter().map(|&i| i as usize).collect();
    println!("Loaded {} selected indices", selected_indices.len());

    // Load all base vectors and pick the random subset
    let all_base = read_fvecs(&data_dir.join("sift_base.fvecs"));
    let base_vectors: Vec<(usize, &Vec<f32>)> = selected_indices
        .iter()
        .map(|&i| (i, &all_base[i]))
        .collect();
    println!("Selected {} base vectors", base_vectors.len());

    // Ingest in batches of 10, using the original sift_base index as external_id
    let num_batches = (base_vectors.len() + 9) / 10;
    for (batch_idx, chunk) in base_vectors.chunks(10).enumerate() {
        let batch: Vec<Vector> = chunk
            .iter()
            .map(|(orig_idx, values)| Vector::new(orig_idx.to_string(), (*values).clone()))
            .collect();
        db.write(batch).await.expect("failed to write batch");
    ...
    let nqueries = 100;
    let queries: Vec<Vec<f32>> = read_fvecs(&data_dir.join("sift_query.fvecs"))
        .into_iter()
        .take(nqueries)
        .collect();
    let ground_truth = read_ivecs(&data_dir.join("sift100k_groundtruth.ivecs"));
    println!(
        "Loaded {} queries and {} ground truth entries",
        queries.len(),
        ground_truth.len()
...
#[tokio::test(flavor = "multi_thread")]
#[ignore] // requires cohere data files (~3GB), slow
async fn cohere1m_recall() {
    init_tracing();
    let data_dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/data/cohere");

    let config = Config {
        dimensions: 768,
        distance_metric: DistanceMetric::Cosine,
        split_threshold_vectors: 1024,
        merge_threshold_vectors: 256,
        ..Default::default()
    };
    let db = VectorDb::open(config)
        .await
    ...
    // Ingest base vectors in batches of 10
    let base_vectors = read_fvecs(&data_dir.join("cohere_base.fvecs"));
    println!(
    ...
    db.flush().await.expect("failed to flush");
    info!("Ingested all base vectors");

    // Wait for rebalancer to quiesce
    tokio::time::sleep(tokio::time::Duration::from_secs(30)).await;

    // Query and measure recall
    let queries = read_fvecs(&data_dir.join("cohere_query.fvecs"));
    let ground_truth = read_ivecs(&data_dir.join("cohere_groundtruth.ivecs"));
    println!(
        "Loaded {} queries and {} ground truth entries",
        queries.len(),
        ground_truth.len()
